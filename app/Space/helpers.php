...
function format_money_pdf($money, $currency = null)
{
    ...
    // Usa Satoshi y deja fallback a DejaVu Sans para símbolos que no tenga Satoshi
    $fontStack = "Satoshi, 'DejaVu Sans', Helvetica, Arial, sans-serif";

    $currency_with_symbol = '';
    if ($currency->swap_currency_symbol) {
        $currency_with_symbol = $format_money.'<span style="font-family: ' . $fontStack . ';">'.$currency->symbol.'</span>';
    } else {
        $currency_with_symbol = '<span style="font-family: ' . $fontStack . ';">'.$currency->symbol.'</span>'.$format_money;
    }
